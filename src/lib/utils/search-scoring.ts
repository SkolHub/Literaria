...
export function filterAndSortStrings(strings: string[], searchTerm: string, aliases: string[] = []): string[] {
  // Score each string based on the search term
  const scoredStrings = strings.map(str => ({
    string: str,
    score: commandScore(str, searchTerm, aliases)
  }));

  // Sort the strings by score in descending order
  scoredStrings.sort((a, b) => b.score - a.score);
